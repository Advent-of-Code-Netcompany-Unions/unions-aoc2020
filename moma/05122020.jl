...
function convert_binary_to_num(binaries)
    num = zeros(size(binaries))

    for (index, value) ∈ enumerate(Iterators.reverse(binaries))
        num[index] = ldexp(convert(Float64, value), index-1)
    end

    return cumsum(num)[end]
end

function seat_string_to_num(seat_code::SubString{String}, comparator::Char)::BitArray
...
function calculate_seat_id(boarding_pass::SubString{String})::NamedTuple{(:row, :col, :id), Tuple{Float64,Float64,Float64}}
...
function board(puzzle_part::Int)
    boarding_passes = split(input, r"\n")
